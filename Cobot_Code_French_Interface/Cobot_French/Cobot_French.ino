...
Point3D basePositions[NB_CYCLES][NB_POSITIONS] = {
  ...
  //position cycle 2
  {
    ...
    {-150, 150, (prehensionHeightBloc)},  //machine A
    {155, 97, prehensionHeight}, //machine B
    {155, 20, prehensionHeight} //convoyeur sortie
  }
  /*OLD absolue
  //position cycle 1
  {
...
const char* configMenu[] = {"Choix Cycle", "Choix Objet", "Choix Mode", "Nb Pieces", "Retour"};
...
void saveSettingsToEEPROM() {
  ...
  Serial.println(F("EEPROM sauvegardee."));
}
...
void resetEEPROMToDefaults() {
  ...
  Serial.println(F("EEPROM reinitialisee aux valeurs par defaut."));
}
...
void drawStatusScreen() {
  ...
  display.print(F("Etat  : ")); display.println(getRunStateText());
...
void printCurrentConfiguration() {
  ...
  Serial.print(F("Cycle selectionne: ")); Serial.println(getCycleText());
  Serial.print(F("Objet selectionne: ")); Serial.println(getObjectText());
  Serial.print(F("Mode: ")); Serial.println(getModeText());
  Serial.print(F("Nombre de pieces: ")); Serial.println(getCombineValue());

  Serial.print(F("X actuel: ")); Serial.println(currentX);
  Serial.print(F("Y actuel: ")); Serial.println(currentY);
  Serial.print(F("Z actuel: ")); Serial.println(currentZ);

  Serial.print(F("Facteur de vitesse: ")); Serial.println(executionSpeedFactor);

  Serial.print(F("Consigne ouverture: ")); Serial.println(eepromData.grip[currentObject].openSetpoint);
  Serial.print(F("Consigne fermeture: ")); Serial.println(eepromData.grip[currentObject].closeSetpoint);

  Serial.print(F("Offset servo A: ")); Serial.println(degOffsetA);
  Serial.print(F("Offset servo B: ")); Serial.println(degOffsetB);
  Serial.print(F("Offset servo Z: ")); Serial.println(degOffsetZ);
  Serial.println(F("==========================================="));
}
...
void printMainMaintenanceMenu() {
  Serial.println();
  Serial.println(F("========== MENU PRINCIPAL MAINTENANCE =========="));

  Serial.println(F("[Configuration]"));
  Serial.println(F("c  - Changer le cycle"));
  Serial.println(F("j  - Changer l objet"));
  Serial.println(F("e  - Changer le mode ECO / Normal / RAPIDE"));
  Serial.println(F("n  - Changer le nombre de pieces"));
  Serial.println(F("f/s - Augmenter / diminuer la vitesse"));

  Serial.println();
  Serial.println(F("[Controle manuel du robot]"));
  Serial.println(F("1 - Servo A manuel avec encodeur"));
  Serial.println(F("2 - Servo B manuel avec encodeur"));
  Serial.println(F("3 - Servo Z manuel avec encodeur"));
  Serial.println(F("8 - Aller en position REPOS"));

  Serial.println();
  Serial.println(F("[Pince]"));
  Serial.println(F("4 - Ouvrir la pince PID"));
  Serial.println(F("5 - Fermer la pince PID"));
  Serial.println(F("6 - Arreter la pince"));

  Serial.println();
  Serial.println(F("[Tables offsets]"));
  Serial.println(F("T1 - Ouvrir table offsets Cycle 1"));
  Serial.println(F("T2 - Ouvrir table offsets Cycle 2"));

  Serial.println();
  Serial.println(F("[Systeme]"));
  Serial.println(F("7 - Tester les LED"));
  Serial.println(F("9 - Afficher la configuration actuelle"));
  Serial.println(F("w - Sauvegarder EEPROM"));
  Serial.println(F("l - Charger EEPROM"));
  Serial.println(F("d - Reinitialiser EEPROM"));
  Serial.println(F("h - Notice"));
  Serial.println(F("m - Afficher le menu"));

  Serial.println(F("==========================================="));
  Serial.println(F("Entrer commande:"));
}

void printSelectedMaintenanceTarget() {
  Serial.println();
  Serial.println(F("----- CIBLE OFFSET SELECTIONNEE -----"));
  Serial.print(F("Cycle: ")); Serial.println(getCycleNameByIndex(maintCycle));
  Serial.print(F("Objet: ")); Serial.println(getObjectNameByIndex(maintObject));
...
void printPositionTable(int cycle, int object) {
  ...
  Serial.print(F(" OFFSET TABLE / Objet: "));
...
void printCycleTableMenu(int cycle) {
  ...
  Serial.print(F("========== "));
  Serial.print(getCycleNameByIndex(cycle));
  Serial.println(F(" MENU TABLE OFFSETS =========="));

  Serial.println(F("[Navigation]"));
  Serial.println(F("j - Change object"));
  Serial.println(F("k - Changer la position"));

  Serial.println();
  Serial.println(F("[Offsets position prehenseur]"));
  Serial.println(F("x/X - Augmenter/diminuer offset X"));
  Serial.println(F("y/Y - Augmenter/diminuer offset Y"));
  Serial.println(F("r/R - Augmenter/diminuer offset Z"));

  Serial.println();
  Serial.println(F("[Offsets angles servos]"));
  Serial.println(F("a/A - Augmenter/diminuer offset Servo A"));
  Serial.println(F("b/B - Augmenter/diminuer offset Servo B"));
  Serial.println(F("z/Z - Augmenter/diminuer offset Servo Z"));

  Serial.println();
  Serial.println(F("[Reglages pince / moteur]"));
  Serial.println(F("p/o - Augmenter/diminuer consigne fermeture"));
  Serial.println(F("i/u - Augmenter/diminuer consigne ouverture"));

  Serial.println();
  Serial.println(F("[Actions]"));
  Serial.println(F("t - Afficher table du cycle"));
  Serial.println(F("w - Sauvegarder EEPROM"));
  Serial.println(F("q - Retour menu principal"));
...
void updateGripSetpoint(bool closeSetpoint, float delta) {
  if (closeSetpoint) {
    ...
    Serial.print(F("Consigne fermeture pour "));
    ...
    Serial.print(F("Consigne ouverture pour "));
    Serial.print(getObjectNameByIndex(maintObject));
    Serial.print(F(" = "));
...
void testLEDsMaintenance() {
  ...
  Serial.println(F("Test LED termine."));
}
...
void updateManualServoFromEncoder() {
  ...
  if (digitalRead(ENC_SW) == LOW) {
    if (millis() - lastEncoderButtonTime > encoderButtonDelay) {
      ...
      Serial.println(F("Controle manuel servo arrete."));
...
void processMainMaintenanceCommand(String cmd) {
  if (cmd == "1") {
    ...
    Serial.println(F("Servo A manuel selectionne. Tourner encodeur. Appuyer ou q pour quitter."));
  }
  else if (cmd == "2") {
    maintConsoleState = MAINT_SERVO_B;
    Serial.println(F("Servo B manuel selectionne. Tourner encodeur. Appuyer ou q pour quitter."));
  }
  else if (cmd == "3") {
    maintConsoleState = MAINT_SERVO_Z;
    Serial.println(F("Servo Z manuel selectionne. Tourner encodeur. Appuyer ou q pour quitter."));
  }
  else if (cmd == "4") {
    applyGripperSettingsForObject(maintObject);
    Serial.println(F("Ouverture pince avec PID..."));
    openGripper();
  }
  else if (cmd == "5") {
    applyGripperSettingsForObject(maintObject);
    Serial.println(F("Fermeture pince avec PID..."));
    closeGripper();
  }
  else if (cmd == "6") {
    Serial.println(F("Arret pince..."));
    stopGripper();
  }
  else if (cmd == "7") {
    testLEDsMaintenance();
    ...
    configChanged();
    Serial.print(F("Cycle selectionne: "));
    ...
    configChanged();
    Serial.print(F("Objet selectionne: "));
    Serial.println(getObjectText());
  }
  else if (cmd == "e") {
    currentMode = (currentMode + 1) % 3;
    configChanged();
    Serial.print(F("Mode selectionne: "));
    Serial.println(getModeText());
  }
  else if (cmd == "n") {
    currentCombine = (currentCombine + 1) % 5;
    configChanged();
    Serial.print(F("Nombre de pieces: "));
    ...
    eepromData.savedSpeedFactor = executionSpeedFactor;
    Serial.print(F("Facteur de vitesse: "));
    ...
    Serial.println(F("Commande inconnue. Taper m pour afficher le menu."));
  }
}
...
void processTableMaintenanceCommand(String cmd) {
  if (cmd == "q" || cmd == "Q") {
    ...
    Serial.println(F("Commande table inconnue. Taper q pour retour menu principal."));
  }
}

void maintenanceMode() {
  drawInfoMessage("MAINTENANCE", "Retour au REPOS", "Utiliser serie");
  ...
  Serial.println();
  Serial.println(F("Entree en mode maintenance..."));
  Serial.println(F("Chargement EEPROM..."));

  initializeFromCartesianPosition(initX, initY, initZ);

  Serial.println(F("Robot en position REPOS."));
  ...
  display.println(F("----------------"));
  display.println(F("Robot au REPOS"));
  display.println(F("Utiliser serie"));
    ...
    if (cmd.length() > 0) {
      if (maintConsoleState == MAINT_SERVO_A ||
        ...
        if (cmd == "q" || cmd == "Q") {
          ...
          Serial.println(F("Tourner encodeur pour bouger servo. Appuyer ou q pour quitter."));
        }
      }
...
void SequenceCycle2ForObject(int objectIndex) {
  ...
  // Machine A re-grab correction: X +3 mm, Y -3 mm.
  moveToPrehensionPositionExtraForObject(1, objectIndex, POS_machineA, +3, -3, 0, 100, tUp, tXY, tDown);
  ...
  // Re-grab Machine B 3 mm in the opposite direction from Machine A.
  // Drop remains unchanged; only this grab-back move is shifted.
  moveToPrehensionPositionExtraForObject(1, objectIndex, POS_machineB, 0, -3, 0, 100, tUp, tXY, tDown);
...
// =====================================================
// PYTHON MODE TERMINAL DISPLAY
// This mode is activated automatically when the Python
// terminal sends PING or any API command. Cavalier inutile.
// =====================================================
void drawTerminalModeScreen() {
  display.clearDisplay();
  display.setTextSize(1);
...
//   GOTO_POSITION cycle object position
...
void apiGotoPosition(String cmd) {
  ...
  if (sp1 < 0 || sp2 < 0) {
    Serial.println(F("ERR;GOTO_POSITION_FORMAT_USE_GOTO_POSITION_CYCLE_OBJECT_POSITION"));
    sendStatus();
    return;
  }

  int cycleIndex = constrain(args.substring(0, sp1).toInt(), 0, NB_CYCLES - 1);
  int objectIndex = constrain(args.substring(sp1 + 1, sp2).toInt(), 0, NB_OBJECTS - 1);
  int positionIndex = constrain(args.substring(sp2 + 1).toInt(), 0, NB_POSITIONS - 1);
